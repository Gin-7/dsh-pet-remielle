...
function mountPet(ctx) {
      ...
      if (ctx && ctx.sessions && ctx.sessions.list && typeof ctx.sessions.list.getSnapshot === 'function') {
        ...
        if (byId) {
          for (var sid in byId) {
            ...
            if (existingIds.has('completion:' + sid)) continue
            sessions.push({
  ...
  function updateBubble(snapshot) {
    ...
    if (cur === 0) {
      ...
      bubble.style.display = (show || bothOn) ? 'block' : 'none'
      if (show) {
  ...
  function applySnapshot(snapshot) {
    ...
    if (snapshot.kind === 'session-action') {
      if (snapshot.sessionId) {
        if (snapshot.approve) approveSession(snapshot.sessionId)
        else openSession(snapshot.sessionId, snapshot.completed === true)
      }
      return
    }
