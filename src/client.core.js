...
function mountPet(ctx) {
  ...
  dock.addEventListener('click', function () {
    ...
    // 延迟 250ms 执行，若紧随其后的双击（画画）到来则取消，避免双击先弹余额。
  ...
  dock.addEventListener('dblclick', function () {
    ...
    // 画画锁：上一张还在画时忽略新的触发，避免多张画面同时抽搐
    if (drawingActive) return
    if (!lastSnapshot || lastSnapshot.pics === 0) return
    drawingActive = true
    // 立即退出余额展示，让「正在创作隐晦物品」立刻显示
    if (window.__petBalance) window.__petBalance.cancel()
    manualOverride = { mood: '01', until: Date.now() + PIC_DRAW_MS }
    sync()
    showPic()
    // 整个画画序列（绘制 + 得意 + 淡出 + 余量）结束后解锁
    window.setTimeout(function () { drawingActive = false }, PIC_DRAW_MS + 3500)
  })
