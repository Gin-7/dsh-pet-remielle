...
function Field(props) {
  return React.createElement('label', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20 } },
      ...
      : React.createElement(React.Fragment, null,
          React.createElement(Field, { label: '启用桌宠', hint: '关闭后宠物立即隐藏。' },
            React.createElement('input', {
              ...
              onChange: function (event) { void write('enabled', event.target.checked) },
            }),
          ),
          ...
          React.createElement(Field, { label: '锁定位置', hint: '开启后宠物不可拖动。' },
            React.createElement('input', {
              ...
              onChange: function (event) { void write('locked', event.target.checked) },
            }),
          ),
          React.createElement(Field, { label: '响应子 Agent', hint: '默认只跟随顶层任务，避免状态过度跳动。' },
            React.createElement('input', {
              ...
              onChange: function (event) { void write('includeSubagents', event.target.checked) },
            }),
          ),
    ...
    React.createElement('div', { style: { flex: 1, minWidth: 0 } },
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' } },
        React.createElement('label', { style: { display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, cursor: 'pointer' } },
          React.createElement('input', {
            type: 'checkbox', checked: pet.enabled === true,
            disabled: !pet.available || busy,
            onChange: function (event) {
              void patchPet(pet.id, { enabled: event.target.checked }).then(function (result) {
                if (result) refresh()
              })
            },
          }),
...
function AddPetForm(props) {
  ...
  return React.createElement('div', {
    style: {
      ...
      borderRadius: 12, display: 'grid', gap: 8,
    },
  },
    React.createElement('strong', { style: { fontSize: 14 } }, '添加新宠物'),
    React.createElement('p', { style: { margin: 0, opacity: 0.7, fontSize: 13 } },
      '把 6 张状态贴纸（01.gif–06.gif）放进插件目录 assets/pets/<id>/，然后在这里登记即可；也可以在放图之前先登记名字。'),
    React.createElement('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
      React.createElement('input', {
        type: 'text', placeholder: '宠物 id（目录名）', value: id,
      ...
      React.createElement('button', { type: 'button', onClick: submit }, '添加并启用'),
    ),
    error ? React.createElement('small', { role: 'alert', style: { color: 'var(--danger-color, #c0392b)' } }, error) : null,
  )
}
...
function PetsSection() {
  ...
  React.useEffect(function () {
    ...
    fetchJson(CONFIG_ENDPOINT)
      ...
      .catch(function () { /* slider just stays disabled */ })
    return function () {
      active = false
      for (var _t of sliderTimers.current.values()) clearTimeout(_t)
      sliderTimers.current.clear()
    }
  }, [])
  var writeScale = function (next) {
    setConfig(function (prev) { return Object.assign({}, prev, { scale: next }) })
    var pending = sliderTimers.current.get('scale')
    if (pending) clearTimeout(pending)
    sliderTimers.current.set('scale', setTimeout(function () {
      sliderTimers.current.delete('scale')
      void patchConfig('scale', next)
    }, 250))
  }
  var sectionStyle = {
    display: 'grid', gap: 10, padding: '4px 2px',
    fontFamily: 'system-ui, sans-serif', fontSize: 13, color: 'var(--dsw-alias-label-primary, #172347)',
  }
  return React.createElement('section', { style: sectionStyle, 'data-testid': 'dsh-pet-remielle-pets-section' },
    React.createElement('h3', { style: { margin: 0, fontSize: 15 } }, '宠物管理'),
    React.createElement('p', { style: { margin: 0, opacity: 0.7 } },
      '管理你的桌宠收藏：启用/禁用、改名、选择当前展示的宠物。新宠物 = 把 6 张贴纸放进 assets/pets/<id>/，然后点"添加"。'),
    error
      ...
      : data === null
      ? React.createElement('span', null, '正在读取宠物列表…')
      : React.createElement(React.Fragment, null,
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '4px 2px 12px' } },
            React.createElement('span', null,
              React.createElement('span', { style: { display: 'block', fontWeight: 600 } }, '角色大小'),
              React.createElement('small', { style: { display: 'block', opacity: 0.65, marginTop: 3 } }, '滑块调整宠物大小（50%–200%），与滚轮缩放联动。'),
            ),
            React.createElement('input', {
              type: 'range', min: 0.5, max: 2, step: 0.05,
              value: config ? (config.scale ?? 1) : 1,
              disabled: !config,
              onChange: function (event) { writeScale(Number(event.target.value)) },
              style: { width: 170 },
            }),
            React.createElement('span', { style: { minWidth: 44, textAlign: 'right', opacity: 0.8 } },
              Math.round((config ? config.scale ?? 1 : 1) * 100) + '%'),
          ),
          React.createElement('div', { style: { display: 'grid', gap: 10 } },
            data.pets.map(function (pet) {
              return petCard(pet, pet.id === data.activePetId, refresh, busy)
            }),
          ),
          data.pets.length === 0
            ? React.createElement('p', { style: { opacity: 0.7 } }, '还没有任何宠物。先添加一只吧！')
            : null,
