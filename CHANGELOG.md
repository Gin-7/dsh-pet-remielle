...
余额与今日已用（借鉴 dsh-whale-widget 的余额/用量思路）：

### Features
- 单击宠物显示「DeepSeek 余额 + 时段」气泡：60 秒自动刷新、单击手动刷新、余额变化数字滚动动画、网络瞬时抖动沿用最近余额不报错；5 秒后自动回到会话状态气泡。
- 今日已用双模式：
  - **小鲸鱼记账**（默认，免令牌）：余额差值累计到 `$DSH_HOME/.dshp-usage.json`，跨天自动归零归档；
  - **实时·令牌**（精确）：配置 `DEEPSEEK_PLATFORM_TOKEN` 后直连平台费用接口 `usage/by_api_key/cost`，直接取平台按小时统计的真实金额（无需本地定价表，DeepSeek 调价自动跟随）。
- 用量模式选择器：设置 → 宠物管理 → 行为，以及桌面窗 / 页面内右键菜单（小鲸鱼记账 / 实时·令牌）。
- 余额气泡美化：「DeepSeek 余额」标签、金额大字号、时段颜色标识（空闲绿 / 高峰红）。
- 平台令牌可直接在「宠物管理 → 行为」里填写（写入 DSH 凭据服务，不存插件配置）。

### Fixes
- 修复余额气泡 5 秒后未回到会话状态内容（状态气泡节流缓存未清空导致余额文字残留）。
- 修复设置中的「暂停动画 / 隐藏桌宠」不生效（状态快照缺 `paused` / `hidden` 字段，现已下发并应用）。
- 移除气泡点击随机台词功能（与原交互冲突）。
- **今日已用金额偏小修复**：旧令牌模式用本地定价表换算，DeepSeek 8 月 18 日 V4 大幅涨价后定价表过时（且 v4-pro 被误按 v4-flash 同价）；改用平台费用接口后金额精确。

### Architecture
- 新增 `src/balance.js`（宿主余额服务：拉取 / 缓存 / 重试 / 记账 / 平台费用接口）与 `src/balance-widget.js`（客户端余额控制器，无 DOM，把显示帧渲染进宠物自带气泡）。
- 宿主注入 `credentials`；状态快照新增 `usageMode` / `paused` / `hidden`；新增 `/balance`、`/platform-token` 与 `/balance-widget.js` 端点。
