...
- 今日已用双模式：
  ...
  - **实时·令牌**（精确）：配置 `DEEPSEEK_PLATFORM_TOKEN` 后直连平台用量接口，按峰谷定价（高峰 9–12、14–18 点）换算。
...
### Architecture
- 新增 `src/balance.js`（宿主余额服务：拉取 / 缓存 / 重试 / 记账 / 峰谷定价）与 `src/balance-widget.js`（客户端余额控制器，无 DOM，把显示帧渲染进宠物自带气泡）。
- 宿主注入 `credentials`；状态快照新增 `usageMode` / `paused` / `hidden`；新增 `/balance` 与 `/balance-widget.js` 端点。
