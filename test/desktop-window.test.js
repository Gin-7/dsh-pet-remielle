...
test('pet-view menu expands to the work-area box and restores on close', () => {
  ...
  // 落点按工作区右→左→上（与网页同一套）；扩窗包围盒含 MENU_GLOW，needT 钳 ≥0
  assert.match(html, /var MENU_GLOW = 16/)
  ...
  assert.match(html, /layoutMenu\(W2, H2, mw, mh, clusterRect\(\)\)/)
  ...
  assert.match(petWindow, /ipcMain\.handle\('menu-expand', \(_event, cssLeft, cssTop, cssRight, cssBottom\)/)
  assert.match(petWindow, /ipcMain\.handle\('menu-restore'/)
  assert.match(petWindow, /dx: Math\.round\(-x \/ uiZoom\)/)
